...
export const WithSelection = () => {
  ...
  const [data] = useState(makeData(200, { aiLabelRows: [1, 3] }));
  ...
  return (
    <TableContainer
      title="AI label, with row selection"
      ...
      <Table size="lg" useZebraStyles={false} aria-label="sample table">
        ...
        <TableBody>
          {table.getRowModel().rows.map((row) => {
            const aiRow = typeof row.original?.aiLabel === 'function';
            return (
              <TableRow
                key={row.id}
                className={cx([
                ...
                {row.getVisibleCells().map((cell, index) => {
                  const slug = cell.row.original?.aiLabel;
                  const SlugComponent =
                    typeof slug === 'function' ? slug : null;
                  return (
                    <React.Fragment key={index}>
                      {index === 0 && (
                        <TableCell className="ai-cell-select">
